...
const API_URL = `${import.meta.env.VITE_API_BASE_URL}/api/admin/drug-import`;
...
const DrugMigrationModal = ({ isOpen, onClose, onSuccess }) => {
    ...
    const handleMigrateSingle = async (drugId) => {
        setMigrating(true);
        setError('');
        try {
            await axios.post(`${API_URL}/import/${drugId}`);
    ...
    const handleMigrateAll = async () => {
        if (!window.confirm(`Import all ${status?.pending} pending drugs from Pharmacy Store?`)) return;

        setMigrating(true);
        setError('');
        try {
            const res = await axios.post(`${API_URL}/import-all`);
            setSuccess(`Successfully imported ${res.data.migrated_count || res.data.imported_count} drugs!`);
            fetchStatus();
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="flex items-center justify-between p-6 border-b border-gray-200 bg-gradient-to-r from-blue-600 to-indigo-600">
                    <div>
                        <h2 className="text-2xl font-bold text-white">Import Medicines from Pharmacy</h2>
                        <p className="text-blue-100 text-sm mt-1">Import existing drugs from the Pharmacy Store into the Main Medicine Store</p>
                    </div>
                    <button
                        onClick={onClose}
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-3 gap-4 mb-6">
                                ...
                                <div className="bg-indigo-50 border border-indigo-200 rounded-lg p-4">
                                    <p className="text-sm font-medium text-indigo-600 mb-1">Available for Import</p>
                            ...
                            {status.pending > 0 ? (
                                <>
                                    ...
                                    >
                                        <Download size={20} />
                                        {migrating ? 'Importing...' : `Import All ${status.pending} Drugs`}
                                    </button>
                                    ...
                                    <div className="border border-gray-200 rounded-lg overflow-hidden">
                                        <div className="bg-blue-50 px-4 py-3 border-b border-blue-200">
                                            <h3 className="font-semibold text-blue-800">
                                                Available for Import ({status.pending_list.length})
                                            </h3>
                                        </div>
                                                    ...
                                                    {status.pending_list.map((drug) => (
                                                        <tr key={drug.id} className="hover:bg-gray-50">
                                                            ...
                                                            <td className="px-4 py-3 text-sm">
                                                                <button
                                                                    onClick={() => handleMigrateSingle(drug.id)}
                                                                    disabled={migrating}
                                ...
                                <div className="text-center py-12">
                                    ...
                                    <p className="text-gray-600">All Pharmacy Store items have been successfully imported into the Main Medicine Store</p>
                                </div>
                            )}
                        </div>
                    )}
